...
for line in data_json:
    if line['是否写入图鉴'] != '':
        ...
        with open('cats/' + line['名字'] + '/' + line['名字'] + '.js', 'w', encoding='utf-8') as f:
            ...
            #  音频数
            if line['是否加音频']:
                audio = '//cloud1-6g5n7d3da0d9860e.636c-cloud1-6g5n7d3da0d9860e-1313177812/' + \
                    line['名字']
...
# 创建毛色分类的js文件
# 奶牛
if not os.path.exists('allcat/奶牛'):
    os.makedirs('allcat/' + '奶牛')  # 创建每只猫的文件夹
    # 创建js文件
with open('allcat/奶牛/奶牛' + '.js', 'w', encoding='utf-8') as f:
    f.write('var app = getApp()\n Page({\ndata: { \n catlist: [\n')
    for name in nainiu:
        f.write('{ name:"'+name+'"},')
...
# 狸花
if not os.path.exists('allcat/狸花'):
    os.makedirs('allcat/' + '狸花')  # 创建每只猫的文件夹
    # 创建js文件
with open('allcat/狸花/狸花' + '.js', 'w', encoding='utf-8') as f:
    f.write('var app = getApp()\n Page({\ndata: { \n catlist: [\n')
    for name in lihua:
        f.write('{ name:"'+name+'"},')
    with open('js2.txt', 'r', encoding='utf-8') as f2:
...
# 玳瑁及三花
if not os.path.exists('allcat/玳瑁及三花'):
    os.makedirs('allcat/' + '玳瑁及三花')  # 创建每只猫的文件夹
    # 创建js文件
with open('allcat/玳瑁及三花/玳瑁及三花' + '.js', 'w', encoding='utf-8') as f:
    f.write('var app = getApp()\n Page({\ndata: { \n catlist: [\n')
    for name in sanhua:
        f.write('{ name:"'+name+'"},')
    with open('js2.txt', 'r', encoding='utf-8') as f2:
...
# 纯色
if not os.path.exists('allcat/纯色'):
    os.makedirs('allcat/' + '纯色')  # 创建每只猫的文件夹
    # 创建js文件
with open('allcat/纯色/纯色' + '.js', 'w', encoding='utf-8') as f:
    f.write('var app = getApp()\n Page({\ndata: { \n catlist: [\n')
    for name in chunse:
        f.write('{ name:"'+name+'"},')
    with open('js2.txt', 'r', encoding='utf-8') as f2:
...
# 橘猫及橘白
if not os.path.exists('allcat/橘猫及橘白'):
    os.makedirs('allcat/' + '橘猫及橘白')  # 创建每只猫的文件夹
    # 创建js文件
with open('allcat/橘猫及橘白/橘猫及橘白' + '.js', 'w', encoding='utf-8') as f:
    f.write('var app = getApp()\n Page({\ndata: { \n catlist: [\n')
    for name in ju:
        f.write('{ name:"'+name+'"},')
    with open('js2.txt', 'r', encoding='utf-8') as f2:
...
# 所有
if not os.path.exists('allcat/所有'):
    os.makedirs('allcat/' + '所有')  # 创建每只猫的文件夹
    # 创建js文件
with open('allcat/所有/所有' + '.js', 'w', encoding='utf-8') as f:
    f.write('var app = getApp()\n Page({\ndata: { \n catlist: [\n')
    for name in suoyou:
        f.write('{ name:"'+name+'"},')
    with open('js2.txt', 'r', encoding='utf-8') as f2:
...
# 创建状态分类的js文件
with open('allcat/allcat' + '.js', 'w', encoding='utf-8') as f:
    f.write('var app = getApp()\n Page({\ndata: { \n')
    #  fostered
